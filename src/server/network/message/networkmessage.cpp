...
// Pre-computed powers of ten, replacing runtime std::pow in addDouble/getDouble.
// NOTE: indices 0..9 only. Every caller must clamp `precision` before indexing —
// getDouble() reads it straight off the wire, so an unclamped index would be an
// out-of-bounds read on a malformed packet.
static constexpr double POW10[] = {
	1.0, 10.0, 100.0, 1000.0, 10000.0,
	100000.0, 1000000.0, 10000000.0, 100000000.0, 1000000000.0
};
static constexpr uint8_t POW10_MAX_PRECISION = 9;
...
std::string NetworkMessage::getString(uint16_t stringLen /* = 0*/, const std::source_location &location) {
	...
	// Fast path: for bytes < 128 ISO-8859-1 and UTF-8 are byte-identical, so the
	// Boost.Locale conversion is a no-op and can be skipped entirely.
	if (std::ranges::none_of(latin1Str, [](unsigned char c) { return c >= 128; })) {
...
void NetworkMessage::skipBytes(int16_t count) {
	if (count < 0) {
		const auto absCount = static_cast<uint16_t>(-count);
...
void NetworkMessage::addString(const std::string &value, const std::source_location &location /*= std::source_location::current()*/, const std::string &function /* = ""*/) {
	...
	// Fast path: for bytes < 128 UTF-8 and ISO-8859-1 are byte-identical, so the
	// Boost.Locale conversion is a no-op and can be skipped entirely.
	std::string latin1Str;
	if (std::ranges::none_of(value, [](unsigned char c) { return c >= 128; })) {
		latin1Str = value;
	} else {
...
void NetworkMessage::addDouble(double value, uint8_t precision /*= 4*/) {
	// POW10 has 10 entries; clamp so an out-of-range precision can never index past it.
	const uint8_t safePrecision = std::min<uint8_t>(precision, POW10_MAX_PRECISION);
	addByte(safePrecision);
	add<uint32_t>((value * POW10[safePrecision]) + std::numeric_limits<int32_t>::max());
}
...
double NetworkMessage::getDouble() {
	...
	// Convert back to the original double value using the pre-computed power of ten.
	// `precision` comes straight off the wire, so it MUST be clamped before indexing POW10.
	const uint8_t safePrecision = std::min<uint8_t>(precision, POW10_MAX_PRECISION);
	return adjustedValue / POW10[safePrecision];
}
...
bool NetworkMessage::canRead(int32_t size) const {
	...
	const int32_t remaining = static_cast<int32_t>(info.length) - (static_cast<int32_t>(info.position) - static_cast<int32_t>(INITIAL_BUFFER_POSITION));
