...
int GameFunctions::luaGameAddInfluencedMonster(lua_State* L) {
	// Game.addInfluencedMonster(monster[, ignoreLimit = false])
	...
	const bool ignoreLimit = Lua::getBoolean(L, 2, false);
	lua_pushboolean(L, g_game().addInfluencedMonster(monster, ignoreLimit));
	return 1;
}
