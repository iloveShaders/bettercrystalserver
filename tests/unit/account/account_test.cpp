...
suite<"account"> accountTest = [] {
	...
	// addPremiumDays no longer mutates the cached snapshot: it goes through
	// AccountRepository::applyPremiumDelta and then reloads. So the starting premium has to be
	// seeded in the repository (setPremiumDays only touches the snapshot) and the account has
	// to be loaded first, otherwise reload() returns NotInitialized and nothing is refreshed.
	test("Account::addPremiumDays sets premium remaining days") = [&injectionFixture] {
		auto [accountRepository] = injectionFixture.get<AccountRepository>();

		Account acc { 1 };
		accountRepository.addAccount("crystal@test.com", AccountInfo { 1, 0, 0, AccountType::ACCOUNT_TYPE_GOD });

		expect(eqEnum(acc.load(), AccountErrors_t::Ok));
		acc.addPremiumDays(100);

		expect(approx(acc.getPremiumLastDay(), getTimeNow() + (100 * 86400), 60 * 60));
		expect(eq(acc.getPremiumRemainingDays(), 100));
	};

	test("Account::addPremiumDays can increase premium") = [&injectionFixture] {
		auto [accountRepository] = injectionFixture.get<AccountRepository>();

		Account acc { 1 };
		accountRepository.addAccount("crystal@test.com", AccountInfo { 1, 50, getTimeNow() + (50 * 86400), AccountType::ACCOUNT_TYPE_GOD });
	...
	test("Account::addPremiumDays can reduce premium") = [&injectionFixture] {
		auto [accountRepository] = injectionFixture.get<AccountRepository>();
		...
		expect(approx(acc.getPremiumLastDay(), getTimeNow() + (20 * 86400), 60 * 60));
		expect(eq(acc.getPremiumRemainingDays(), 20));
	};

	test("Account::addPremiumDays keeps the snapshot unchanged if the repository fails") = [&injectionFixture] {
		auto [accountRepository] = injectionFixture.get<AccountRepository>();
		...
		expect(eqEnum(acc.load(), AccountErrors_t::Ok));
		accountRepository.failApplyPremiumDelta = true;
		acc.addPremiumDays(100);
